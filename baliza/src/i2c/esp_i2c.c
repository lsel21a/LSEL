...
uint8_t i2c_read_data(uint8_t dev_addr, uint8_t reg_addr, uint8_t *data, uint8_t data_size, uint32_t timeout){
    ...
    rslt = i2c_driver_install(I2C_PORT, I2C_MODE_MASTER, 0, 0, 0);
    if (rslt != ESP_OK){
        printf("Instalación de drivers[read].\n");
        check_rslt(rslt);
    }
    ...
    rslt = i2c_master_start(cmd);
    if (rslt != ESP_OK){
        printf("Inicialización en modo máster[read].\n");
        check_rslt(rslt);
    }
    ...
    rslt = i2c_master_write_byte(cmd, (dev_addr << 1) | I2C_MASTER_WRITE , true);
    if (rslt != ESP_OK){
        printf("Escritura de la dirección del esclavo[read].\n");
        check_rslt(rslt);
    }

    // Se escriben los datos que se deseen
    rslt = i2c_master_write(cmd, (uint8_t *) &reg_addr, 1, true);
    if (rslt != ESP_OK){
        printf("Escritura de datos[read].\n");
        check_rslt(rslt);
    }
    ...
    // Se leen los datos que se deseen (n - 1) con ACK
    if (data_size > 1){
        rslt = i2c_master_read(cmd, (uint8_t *) data, (uint8_t)(data_size-1), true);
        rslt = i2c_master_cmd_begin(I2C_PORT, cmd, timeout/portTICK_PERIOD_MS);
        if (rslt != ESP_OK){
            printf("Lectura de datos con ACK[read].\n");
            check_rslt(rslt);
        }
    }
    ...
    rslt = i2c_master_read_byte(cmd, (uint8_t *) &data[data_size-1], false);
    if (rslt != ESP_OK){
        printf("Lectura de datos sin ACK[read].\n");
        check_rslt(rslt);
    }
    ...
    rslt = i2c_master_stop(cmd);
    if (rslt != ESP_OK){
        printf("Finalización del comando[read].\n");
        check_rslt(rslt);
    }
    ...
    rslt = i2c_master_cmd_begin(I2C_PORT, cmd, timeout/portTICK_PERIOD_MS);
    if (rslt != ESP_OK){
        printf("Ejecución del comando[read].\n");
        check_rslt(rslt);
    }
